...
for i in 1:(length(homology_degrees)-1)
    if homology_degrees[i+1] == homology_degrees[i]+1
        push!(consecutive_degrees,(homology_degrees[i],homology_degrees[i+1]))
    end
end
for pair in consecutive_degrees
    n = pair[2]+1
...
# We may miss extreme Laplacians which require only one differential to be defined.
# One can consider them separately instead.
Δ = Dict()
for pair in consecutive_degrees
    n = pair[1]+1
    d_n = LowCohomologySOS.embed.(identity, d[n], Ref(RG_Δ[n]))
    d_n_plus_1 = LowCohomologySOS.embed.(identity, d[n+1], Ref(RG_Δ[n]))
    Δ[n-1] = d_n'*d_n+d_n_plus_1*d_n_plus_1'+stab_part_dim[n]
    Δ[n-1] = LowCohomologySOS.embed.(identity, Δ[n-1], Ref(RG_Δ_star[n]))
end
