...
struct EuclideanAlgorithm
    ...
    function EuclideanAlgorithm(sln)
        new(sln)
    end
    function gelt_from_matrix(M::AbstractMatrix)
        inv_result, temp_matrix = one(sln), copy(M)
        for i in 1:(N-1)
            factor, temp_matrix = reduce_column_down(i,temp_matrix)
            inv_result = factor*inv_result
        end
        for i in N:-1:2
            factor, temp_matrix = reduce_column_up(i,temp_matrix)
            inv_result = factor*inv_result
        end
        result = inv_result^(-1)
        return result
    end
    function reduce_column_up(col::Integer, M::AbstractMatrix)
        result_factor, result_matrix = one(sln), copy(M)
        for k in (col-1):-1:1
            mult = div(M[k,col],M[col,col])
            factor, result_matrix = add_row_multiplicity(k,col,-mult,result_matrix)
            result_factor = factor*result_factor
        end
        return result_factor, result_matrix
    end
    function reduce_column_down(col::Integer, M::AbstractMatrix)
        result_factor, result_matrix = one(sln), copy(M)
        for k in (col+1):N
            factor, result_matrix = euclidean_algorithm(col,k,col,result_matrix)
            result_factor = factor*result_factor
        end
        return result_factor, result_matrix
    end
    function euclidean_algorithm(i_row::Integer, j_row::Integer, col::Integer, M::AbstractMatrix)
        result_factor, result_matrix = one(sln), copy(M)
        it = 0
        while result_matrix[i_row,col] != 1 && result_matrix[i_row,col] != -1 && it < 10
            if abs(result_matrix[i_row,col]) <= abs(result_matrix[j_row,col]) && result_matrix[i_row,col] != 0
                ...
                factor, result_matrix = add_row_multiplicity(j_row,i_row,-mult,result_matrix)
                result_factor = factor*result_factor
            elseif result_matrix[j_row,col] != 0
                factor, result_matrix = flip_two_rows(i_row,j_row,result_matrix)
                result_factor = factor*result_factor
            end
            it += 1
        end
        ...
        if result_matrix[i_row,col] != 1 # we want 1 instead of -1 in the ith row
            factor, result_matrix = invert_two_rows(i_row,j_row,result_matrix)
            result_factor = factor*result_factor
        end
        return result_factor, result_matrix
    end
    function add_row_multiplicity(i::Integer, j::Integer, mult::Integer, M::AbstractMatrix)
        if mult != -1 && mult != 1
    ...
    function flip_two_rows(i::Integer, j::Integer, M::AbstractMatrix)
    ...
    function invert_two_rows(i::Integer, j::Integer, M::AbstractMatrix)
    ...
    function E(i::Integer, j::Integer)
